...
describe 'Editor' do
  ...
  let!(:site_moderator) { create :site_editor, :site => site, :user => current_user }

  let!(:another_rubric) { create :doc_rubric, :site => site }
  let!(:another_doc) { create :doc, :approved, :site => site, :rubric => doc_rubric }
  let!(:another_rubric_doc) { create :doc, :approved, :site => site, :rubric => another_rubric }
  ...
  context 'with rubric permissions' do
    ...
    context 'on rubric page' do
      before do
        visit "/news/#{doc_rubric.link}"
      end

      it 'sees articles' do
        find('div.b-items-list').all('div.b-items-list__item').should have(2).items
      end
      ...
      it 'sees link to edit and delete another article' do
      ...
      it 'contains links to new articles' do
        #page.should have_link('От юзеров') - в Воронеже нет ссылки "от юзеров"
    ...
    context 'on article page' do
      ...
      context 'when not approved' do
        ...
        it 'sees link to manage article' do
          page.should have_link('редактировать')
          page.should have_link('подтвердить')
          page.should have_link('удалить')
        end
      end
    end
    ...
    context 'when edit article' do
      it 'see rich textarea', :js => true do
        visit_with_port("/news/#{doc.id}/edit")
        page.should have_css('span#cke_doc_text')
      end

      it 'sees approved checkbox' do
        visit("/news/#{doc.id}/edit")
        page.should have_css('input#doc_approved')
      end
    end

    context 'when creates article' do
      it 'see rich textarea', :js => true do
        visit_with_port("/news/new")
        page.should have_css('span#cke_doc_text')
      end
  ...
  context 'without rubric permissions' do
    context 'on rubric page' do
      ...
      it 'does not see link to edit and delete for article' do
        within("div#doc_#{doc.id}") do
    ...
    context 'on article page' do
      before { visit "/news/#{another_rubric.link}/#{another_rubric_doc.id}" }
    ...
    context 'when edit article' do
      ...
      it 'does not see rich textarea' do
        page.should_not have_css('span#cke_news_doc_text')
      end
